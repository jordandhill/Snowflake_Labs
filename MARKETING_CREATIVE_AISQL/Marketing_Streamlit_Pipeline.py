...
st.title("🎨 Marketing Creative Review")

# ---------- Snowflake Session & Data ---------- #
session = get_active_session()


def fetch_pending_creatives():
    """Pull all rows that are not yet approved and attach image bytes."""
    df = (
        session.table("CREATIVE_IMAGE_REVIEW")
        #.filter(col("STATUS") != "Approved (manual)")
        .to_pandas()
    )
...
# Load data (cached for 60 s)
df = fetch_pending_creatives()

if df.empty:
    st.success("All caught up — nothing to review right now! 🎉")
    st.stop()
